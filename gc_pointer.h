...
template<class T, int size>
    ...
    {
        // Check if this Pointer is in use, reduce refcount if yes
        typename std::list<PtrDetails<T> >::iterator p = findPtrInfo(addr);
        if (p != refContainer.end())
        {
            p->refcount--;
        }
        // Check if t is already used by another Pointer with the same size
        p = findPtrInfo(t);
        if (p != refContainer.end() && p->arraySize == arraySize)
        {
            p->refcount++;
        }
        else
